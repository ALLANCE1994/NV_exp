...
# 数据保存文件夹路径：
savePath = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "data", "Saved_Data") + os.sep
# 数据文件文件名
...
# FPGA 参数计算（用于 USB-7855R）
# 调用 connectionConfig.py 中的函数计算 FPGA 同步控制器参数
if DAQ_DEVICE_TYPE == 1:
    FPGA_DAQ_DELAY, FPGA_DAQ_DURATION = calculateFPGAparams(t_readoutDelay, Nsamples)
else:
    FPGA_DAQ_DELAY = 0
    FPGA_DAQ_DURATION = 0
